...
namespace RogueliteToolkit.SkillTree.Editor
{
    public static class SkillNodeAssetMigration
    {
        ...
        public static void MigrateAll()
        {
            ...
            {
                SkillTreeDefinition tree = AssetDatabase.LoadAssetAtPath<SkillTreeDefinition>(AssetDatabase.GUIDToAssetPath(guid));
                MigrateTree(tree);
            }
        }
            ...
            SkillNodeAuthoring[] authored)
        {
            for (int i = 0; i < nodes.Length; i++)
            {
                ...
                {
                    Undo.RecordObject(data, "Bake Skill Node Data");
                    data.CopyFrom(node);
