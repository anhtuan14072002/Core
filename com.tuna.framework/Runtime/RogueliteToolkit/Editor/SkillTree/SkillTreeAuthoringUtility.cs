    ...
    internal static class SkillTreeAuthoringUtility
    {
            ...
            SkillTreeAuthoring tree, Vector2? anchoredPosition = null)
        {
            ...
            gameObject.transform.SetParent(tree.NodesRoot != null
                ? tree.NodesRoot : tree.transform, false);
            ...
            SkillTreeAuthoring tree, SkillNodeAuthoring source)
        {
            ...
            SetNodeIdentity(node, id,
                string.IsNullOrWhiteSpace(source.DisplayName)
                    ? "Skill Node" : source.DisplayName + " Copy", false);
            clone.name = id;
            ...
            int minimumLevel, out string error)
        {
            ...
            GameObject gameObject = tree.ConnectionPrefab != null
                ? UnityEngine.Object.Instantiate(tree.ConnectionPrefab.gameObject)
                : new GameObject($"{from.Id} -> {to.Id}", typeof(RectTransform),
                    typeof(CanvasRenderer), typeof(Image),
            ...
            gameObject.transform.SetParent(tree.ConnectionsRoot != null
                ? tree.ConnectionsRoot : tree.transform, false);
            ...
            Image image = gameObject.GetComponent<Image>();
            if (image != null && tree.ConnectionPrefab == null)
            {
        ...
        internal static bool TryRefreshFromDefinition(SkillTreeAuthoring tree, out string error)
        {
            ...
            try
            {
                ...
                foreach (SkillConnectionAuthoring connection in tree.GetComponentsInChildren<SkillConnectionAuthoring>(true))
                    Undo.DestroyObjectImmediate(connection.gameObject);
                foreach (SkillNodeAuthoring node in tree.GetComponentsInChildren<SkillNodeAuthoring>(true))
                    Undo.DestroyObjectImmediate(node.gameObject);
                ...
                foreach (SkillConnectionDefinition source in definition.Connections)
                {
                    ...
                    else if (source.Requirement is NodeUnlockedRequirement unlocked && unlocked.NodeId == source.FromNodeId)
                        condition = SkillConnectionCondition.Unlocked;
                    else
                        throw new InvalidOperationException("This definition contains a requirement the authoring tool cannot represent.");
        ...
        internal static void Bake(SkillTreeAuthoring authoring)
        {
            ...
            SkillNodeAssetMigration.StoreNodes(output, nodes,
                authoring.GetComponentsInChildren<SkillNodeAuthoring>(true));
        ...
        internal static int[] NormalizeCosts(IReadOnlyList<int> source, int maxLevel)
        {
            int[] result = new int[Math.Max(1, maxLevel)];
            int lastCost = 0;
        ...
        private static string GenerateUniqueId(SkillTreeAuthoring tree, string seed)
        {
            string normalized = string.IsNullOrWhiteSpace(seed)
                ? "skill_node" : seed.Trim().ToLowerInvariant().Replace(' ', '_');
            HashSet<string> ids = new(StringComparer.Ordinal);
            SkillNodeAuthoring[] nodes =
