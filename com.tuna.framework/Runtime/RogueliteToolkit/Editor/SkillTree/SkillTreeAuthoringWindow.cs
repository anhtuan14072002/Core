    ...
    public sealed class SkillTreeAuthoringWindow : EditorWindow
    {
        ...
        private void Bake()
        {
            if (RequireTree())
            {
                ...
                RebuildGraph(); // Rebind inline fields to the newly assigned node assets.
            }
        }
            ...
            internal void Populate(SkillTreeAuthoring tree)
            {
                ...
                try
                {
                    ...
                    SkillNodeAuthoring[] authoredNodes =
                        tree.GetComponentsInChildren<SkillNodeAuthoring>(true);
        ...
        private sealed class SkillNodeElement : Node
        {
                ...
                SkillNodeAuthoring authoring, SkillTreeAuthoringWindow window)
            {
                ...
                title = string.IsNullOrWhiteSpace(authoring.DisplayName)
                    ? authoring.Id : authoring.DisplayName;
            ...
            private void BuildSummary()
            {
                ...
                dragHandle.style.width = 24f;
                dragHandle.style.minWidth = 24f;
                dragHandle.style.flexShrink = 0f;
                ...
                dragHandle.RegisterCallback<MouseDownEvent>(evt =>
                {
                    if (evt.button != 0)
                        return;
                    draggedNodes.Clear();
                    foreach (SkillNodeElement node in GetFirstAncestorOfType<GraphView>().selection.OfType<SkillNodeElement>())
                    {
                ...
                void FinishDrag()
                {
                    ...
                    Undo.RecordObjects(draggedNodes.Select(entry => (UnityEngine.Object)entry.node.Authoring.RectTransform).ToArray(), "Move Skill Nodes");
                    foreach (var entry in draggedNodes)
                    {
                ...
                displayName.style.unityTextAlign = TextAnchor.MiddleLeft;
                titleContainer.Add(displayName);
                ...
                mainContainer.Insert(3, _collapsedIdentity);
                ...
                void RefreshSummary()
                {
                    ...
                    foreach (StatGameplayEffect effect in statEffects)
                    {
                        string label = statEffects.Length > 1
                            ? $"Value ({(effect.Stat != null ? effect.Stat.name : "None")})" : "Value";
                        rows.Add(label + ": " + string.Join(", ", Enumerable.Range(1, Authoring.MaxLevel)
                            .Select(level => (effect.GetValueAtLevel(level) - effect.GetValueAtLevel(level - 1)).ToString("0.###"))));
                    }
            ...
            private Rect AlignWithOtherNodes(Rect moved, bool snap)
            {
                ...
                foreach (SkillNodeElement other in graph.nodes.ToList().OfType<SkillNodeElement>())
                {
                    ...
                    for (int i = 0; i < 3; i++)
                    {
                        for (int j = 0; j < 3; j++)
                        {
                            ...
                            {
                                bestX = Mathf.Abs(dx); deltaX = dx; lineX = tx[j];
                                targetX = target; foundX = true;
                            }
                            float dy = ty[j] - ys[i];
                            if (Mathf.Abs(dy) < bestY)
                            {
                                bestY = Mathf.Abs(dy); deltaY = dy; lineY = ty[j];
                                targetY = target; foundY = true;
                            }
                        }
                    }
                }
                ...
                if (foundX)
                    AddAlignmentGuide(new Rect(lineX, Mathf.Min(moved.yMin, targetX.yMin) - padding,
                        1f / zoom, Mathf.Max(moved.yMax, targetX.yMax) - Mathf.Min(moved.yMin, targetX.yMin) + padding * 2f));
                if (foundY)
                    AddAlignmentGuide(new Rect(Mathf.Min(moved.xMin, targetY.xMin) - padding, lineY,
                        Mathf.Max(moved.xMax, targetY.xMax) - Mathf.Min(moved.xMin, targetY.xMin) + padding * 2f, 1f / zoom));
                return moved;
            }
            ...
            private void OnNodeGeometryChanged(GeometryChangedEvent evt)
            {
                ...
                if (Authoring.EditorExpanded != expanded)
                {
                    Undo.RecordObject(Authoring, expanded
                        ? "Expand Skill Node" : "Collapse Skill Node");
            ...
            private void ApplyExpandedLayout()
            {
                ...
                VisualElement identityParent = expanded
                    ? _identityExpandedParent : _collapsedIdentity;
                ...
                else
                {
                    ...
                    style.minWidth = 224f;
                    style.maxWidth = 224f;
                    style.minHeight = 178f;
                    style.maxHeight = 178f;
