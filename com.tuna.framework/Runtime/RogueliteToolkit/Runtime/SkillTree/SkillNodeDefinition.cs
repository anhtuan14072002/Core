...
namespace RogueliteToolkit.SkillTree
{
    ...
    public sealed class SkillNodeDefinition
    {
        ...
        [SerializeField, HideInInspector] private int[] _costs = Array.Empty<int>();
        ...
        public IReadOnlyList<int> Costs => _data != null ? _data.Costs : _costs ?? Array.Empty<int>();
        ...
        // Phases start at zero stars. End levels are promotion thresholds,
        ...
        public string GetProgressLabel(int totalLevel) => PhaseCount == 1
            ...
            : $"{GetLevelInPhase(totalLevel)}/{GetPhaseLevelCount(GetPhase(totalLevel))} | {GetPhase(totalLevel)} sao";
        ...
        public int GetCostForLevel(int level)
        {
            ...
            return level <= Costs.Count
                ? Mathf.Max(0, Costs[level - 1])
                : 0;
        }

        public SkillTreeResourcesType GetCostResourceTypeForLevel(int level)
        {
            if (level < 1 || level > MaxLevel)
                throw new ArgumentOutOfRangeException(nameof(level));
            ...
            Color? color = null)
        {
            ...
            _costs = costs != null ? (int[])costs.Clone() : Array.Empty<int>();
