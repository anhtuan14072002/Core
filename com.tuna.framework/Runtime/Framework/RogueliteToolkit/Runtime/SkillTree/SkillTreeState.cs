...
namespace RogueliteToolkit.SkillTree
{
    public sealed class SkillTreeState
    {
            ...
            SkillNodeDefinition node, GameplayEffectContext context)
        {
            ...
            for (int i = 0; i < node.CostResourceCount; i++)
            {
                double cost = node.GetCostForLevel(currentLevel + 1, i);
                if (cost <= 0) continue;
                if (context == null || !context.TryGet(out ISkillPointWallet wallet) ||
                    !wallet.CanSpend(cost, node.GetCostResourceTypeForLevel(currentLevel + 1, i)))
                    return false;
            }
            return true;
        }
            ...
            SkillNodeDefinition node, GameplayEffectContext context)
        {
            ...
            bool hasCost = false;
            for (int i = 0; i < node.CostResourceCount; i++)
                hasCost |= node.GetCostForLevel(currentLevel, i) > 0;
            if (hasCost && !context.GetRequired<ISkillPointWallet>().TrySpend(node, currentLevel))
                return false;
