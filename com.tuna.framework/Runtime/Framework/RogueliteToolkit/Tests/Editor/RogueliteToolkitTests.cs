...
namespace RogueliteToolkit.Tests
{
    ...
    {
        [Test]
        public void SkillTreeAuthoring_BakePreservesView_ResetIsExplicitAndUndoable()
        {
            string folder = "Assets/SkillTreeBakeTest_" + Guid.NewGuid().ToString("N");
            AssetDatabase.CreateFolder("Assets", folder.Substring("Assets/".Length));
            GameObject root = new("Bake View Test", typeof(RectTransform), typeof(SkillTreeAuthoring));
            ...
            try
            {
                ...
                SetField(tree, "_id", "bake_view_test");
                SkillTreeDefinition output = ScriptableObject.CreateInstance<SkillTreeDefinition>();
                AssetDatabase.CreateAsset(output, folder + "/Tree.asset");
                tree.SetOutput(output);
                GameObject child = new("Node", typeof(RectTransform), typeof(SkillNodeAuthoring));
                child.transform.SetParent(root.transform, false);
                SkillNodeAuthoring node = child.GetComponent<SkillNodeAuthoring>();
                SetField(node, "_id", "node");
                SetField(node, "_costs", new[] { 7d });
                SetField(node, "_costResourceType", SkillTreeResourcesType.Coal);
                MethodInfo addResource = typeof(SkillTreeAuthoringWindow).Assembly
                    .GetType("RogueliteToolkit.SkillTree.Editor.SkillNodeDataInspector")
                    .GetMethod("AddResourceColumn", BindingFlags.Static | BindingFlags.NonPublic);
                using (SerializedObject serialized = new(node))
                {
                    addResource.Invoke(null, new object[] { serialized, 1 });
                    SerializedProperty extra = serialized.FindProperty("_additionalCosts").GetArrayElementAtIndex(0);
                    extra.FindPropertyRelative("_resource").intValue = (int)SkillTreeResourcesType.Diamond;
                    extra.FindPropertyRelative("_costs").GetArrayElementAtIndex(0).doubleValue = 11d;
                    addResource.Invoke(null, new object[] { serialized, 1 });
                    SerializedProperty added = serialized.FindProperty("_additionalCosts").GetArrayElementAtIndex(1);
                    Assert.That(added.FindPropertyRelative("_resource").intValue, Is.EqualTo(0));
                    Assert.That(added.FindPropertyRelative("_costs").GetArrayElementAtIndex(0).doubleValue, Is.Zero);
                    added.FindPropertyRelative("_resource").intValue = (int)SkillTreeResourcesType.Quartz;
                    added.FindPropertyRelative("_costs").GetArrayElementAtIndex(0).doubleValue = 13d;
                    serialized.ApplyModifiedPropertiesWithoutUndo();
                }
                node.RectTransform.anchoredPosition = new Vector2(517f, 239f);
                node.SetEditorSize(new Vector2(530f, 370f));
                node.SetEditorExpanded(false);
                SetField(window, "_tree", tree);
                window.CreateGUI();
                GraphView graph = window.rootVisualElement.Q<GraphView>();
                Node card = graph.nodes.Single();
                graph.AddToSelection(card);
                graph.UpdateViewTransform(new Vector3(23f, 47f, 0f), Vector3.one * 1.5f);
                Rect rect = card.GetPosition();
                for (int i = 0; i < 3; i++)
                {
                    SkillNodeDataSO existingData = node.Data;
                    if (existingData != null)
                        SetField(existingData, "_displayName", string.Empty);
                    string beforeBake = existingData != null ? EditorJsonUtility.ToJson(existingData) : null;
                    if (i == 2)
                    {
                        node.SetData(null);
                        SetField(node, "_costs", new[] { 0d });
                        SetField(node, "_additionalCosts", Array.Empty<SkillResourceCost>());
                    }
                    Invoke(window, "Bake");
                    if (existingData != null)
                    {
                        Assert.That(node.Data, Is.SameAs(existingData));
                        Assert.That(EditorJsonUtility.ToJson(node.Data), Is.EqualTo(beforeBake));
                    }
                    Assert.That(graph.nodes.Single(), Is.SameAs(card));
                    Assert.That(graph.selection.Contains(card), Is.True);
                    Assert.That(card.GetPosition(), Is.EqualTo(rect));
                    Assert.That(node.EditorExpanded, Is.False);
                    Assert.That(node.EditorSize, Is.EqualTo(new Vector2(530f, 370f)));
                    Assert.That(graph.viewTransform.scale.x, Is.EqualTo(1.5f));
                    Assert.That(graph.viewTransform.position, Is.EqualTo(new Vector3(23f, 47f, 0f)));
                    Assert.That(output.Nodes[0].Costs[0], Is.EqualTo(7d));
                    Assert.That(output.Nodes[0].CostResourceCount, Is.EqualTo(3));
                    Assert.That(output.Nodes[0].GetCostResourceTypeForLevel(1, 1), Is.EqualTo(SkillTreeResourcesType.Diamond));
                    Assert.That(output.Nodes[0].GetCostForLevel(1, 1), Is.EqualTo(11d));
                    Assert.That(output.Nodes[0].GetCostResourceTypeForLevel(1, 2), Is.EqualTo(SkillTreeResourcesType.Quartz));
                    Assert.That(output.Nodes[0].GetCostForLevel(1, 2), Is.EqualTo(13d));
                    Assert.That(node.Data, Is.SameAs(output.Nodes[0].Data));
                }
                Assert.That(window.rootVisualElement.Query<ToolbarButton>().ToList()
                    .Any(button => button.text == "Reset Node View"), Is.True);
                Undo.FlushUndoRecordObjects();
                Undo.IncrementCurrentGroup();
                Invoke(window, "ResetNodeView");
                Undo.FlushUndoRecordObjects();
                Assert.That(node.EditorExpanded, Is.True);
                Assert.That(node.EditorSize, Is.EqualTo(new Vector2(320f, 240f)));
                Assert.That(node.RectTransform.anchoredPosition, Is.EqualTo(new Vector2(517f, 239f)));
                Assert.That(node.Costs[0], Is.EqualTo(7d));
                Undo.PerformUndo();
                Assert.That(node.EditorExpanded, Is.False);
                Assert.That(node.EditorSize, Is.EqualTo(new Vector2(530f, 370f)));
            }
        ...
        public void ExcelPaste_SelectsNamedColumnAndMapsLevels()
        {
            ...
            Assert.That(SkillNodeTablePaste.TryParse("1.5\n5.11004E30", "Price", 2, out prices, out _), Is.True);
            Assert.That(prices[2], Is.EqualTo(5.11004e30));
            Assert.That(SkillNodeTablePaste.TryParse("1E40", "Value", 1, out _, out _), Is.False);
        }
        ...
        public void UnevenPhases_PreserveBoundariesAndPricesThroughDataAndAuthoring()
        {
            ...
            try
            {
                ...
                Assert.That(node.GetProgressLabel(12), Is.EqualTo("4/4 | 3 sao"));
        ...
        public void CostResourceType_MigratesFirstLegacyResourceForAllLevels()
        {
            ...
            SetField(node, "_costResourceTypes", new[] { SkillTreeResourcesType.Dirt, SkillTreeResourcesType.Emerald });
            SkillNodeDataSO data = ScriptableObject.CreateInstance<SkillNodeDataSO>();
            try
            {
                data.CopyFrom(node);
        ...
        [Test]
        public void SkillNodeColorType_IsPreservedInNodeData()
        {
            SkillNodeDefinition node = new();
            node.Configure("purple", "purple", null, null, Vector2.zero, 1,
                new[] { 1 }, RequirementMode.All, Array.Empty<StatGameplayEffect>(),
                Array.Empty<GameplayEffect>(), nodeColor: SkillNodeColor.Purple);
            SkillNodeDataSO data = ScriptableObject.CreateInstance<SkillNodeDataSO>();
            try
            {
                data.CopyFrom(node);
                node.SetData(data);
                Assert.That(node.NodeColor, Is.EqualTo(SkillNodeColor.Purple));
                Assert.That(data.NodeColor, Is.EqualTo(SkillNodeColor.Purple));
            }
            finally
            {
                UnityEngine.Object.DestroyImmediate(data);
        ...
        [Test]
        public void InstantFreeUpgrade_PreservesRequirementsLimitsAndConfiguredPrices()
        {
            var root = Node("free_root", 1, new[] { 0 });
            var target = Node("free_target", 2, new[] { 50, 100 });
            var data = ScriptableObject.CreateInstance<SkillNodeDataSO>();
            var copy = ScriptableObject.CreateInstance<SkillNodeDataSO>();
            var definition = ScriptableObject.CreateInstance<SkillTreeDefinition>();
            try
            {
                data.CopyFrom(target);
                target.SetData(data);
                var serialized = new SerializedObject(data);
                serialized.FindProperty("_instantFreeUpgrade").boolValue = true;
                serialized.ApplyModifiedPropertiesWithoutUndo();
                copy.CopyFrom(target);
                Assert.That(copy.InstantFreeUpgrade, Is.True);
                definition.Configure("free_test", new[] { root, target }, new[]
                {
                    new SkillConnectionDefinition(root.Id, target.Id, new NodeUnlockedRequirement(root.Id))
                });
                var wallet = new TestWallet(0);
                var context = new GameplayEffectContext().Register<ISkillPointWallet>(wallet);
                var state = new SkillTreeState(definition);
                Assert.That(state.Purchase(target, context), Is.False, "Locked nodes must stay locked.");
                Assert.That(state.Purchase(root, context), Is.True);
                Assert.That(state.Purchase(target, context), Is.True);
                Assert.That(state.GetLevel(target), Is.EqualTo(1));
                Assert.That(wallet.Points, Is.Zero);
                serialized.Update();
                serialized.FindProperty("_instantFreeUpgrade").boolValue = false;
                serialized.ApplyModifiedPropertiesWithoutUndo();
                Assert.That(target.GetCostForLevel(2), Is.EqualTo(100));
                Assert.That(state.Purchase(target, context), Is.False, "Unchecked nodes must require payment.");
                serialized.FindProperty("_instantFreeUpgrade").boolValue = true;
                serialized.ApplyModifiedPropertiesWithoutUndo();
                Assert.That(state.Purchase(target, context), Is.True);
                Assert.That(state.Purchase(target, context), Is.False, "Max level must still apply.");
                Assert.That(wallet.Points, Is.Zero);
                Assert.That(data.Costs[0], Is.EqualTo(50));
                var save = new SkillTreeSaveData();
                state.WriteSaveData(save);
                var restored = new SkillTreeState(definition);
                restored.Restore(save, context);
                Assert.That(restored.GetLevel(target), Is.EqualTo(2));
            }
            finally
            {
                UnityEngine.Object.DestroyImmediate(definition);
        ...
        private sealed class TestWallet : ISkillPointWallet
        {
            ...
            public bool TrySpend(SkillNodeDefinition node, int level)
            {
                double total = 0;
                for (int i = 0; i < node.CostResourceCount; i++) total += node.GetCostForLevel(level, i);
                return TrySpend(total);
            }

            public TestWallet(int points) => Points = points;
            public bool CanSpend(double amount, SkillTreeResourcesType resourceType = SkillTreeResourcesType.None) => amount >= 0 && Points >= amount;

            public bool TrySpend(double amount, SkillTreeResourcesType resourceType = SkillTreeResourcesType.None)
            {
                if (!CanSpend(amount))
                    return false;
                Points -= amount;
